...
import { honourSiftaRedeem, listSiftaRedeems, type SiftaRedeem } from "@/lib/sifta-loot";
...
function StaffHub() {
  ...
  const [loot, setLoot] = useState<SiftaRedeem[]>([]);
  ...
  useEffect(() => {
    ...
    void listSiftaRedeems({ data: pin }).then(setLoot);
  }, [isOwner, pin]);
      ...
      <div className="mx-auto max-w-4xl text-center">
        ...
        {isOwner ? (
          <section className="mx-auto mt-12 max-w-lg rounded-2xl border border-cream/10 p-5 text-left text-sm">
            ...
            <h3 className="mt-6 font-display text-base font-bold">SiftaLoot codes</h3>
            <p className="mt-1 text-cream/60">
              Members redeem on the site (no Sifta Browser). Honour in store / Trade Me.
            </p>
            <ul className="mt-2 space-y-2">
              {loot.length === 0 ? (
                <li className="text-cream/50">None yet.</li>
                ...
                loot.map((row) => (
                  <li key={row.id} className="rounded-xl bg-cream/5 p-3">
                    <p className="font-mono font-medium">
                      {row.code} · {Math.abs(row.amount)} SL · {row.status}
                    </p>
                    {row.status === "pending" ? (
                      <button
                        type="button"
                        className="mt-2 rounded-full bg-cream/15 px-3 py-1 text-xs"
                        onClick={() => {
                          if (!pin) return;
                          void honourSiftaRedeem({ data: { pin, id: row.id } }).then(() =>
                            listSiftaRedeems({ data: pin }).then(setLoot),
                          );
                        }}
                      >
                        Mark honoured
