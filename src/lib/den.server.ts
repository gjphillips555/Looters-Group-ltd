...
export const denMe = createServerFn({ method: "GET" }).handler(async () => {
  ...
  if (session.kind === "admin") {
    return { role: "admin" as const, user: { id: "admin", username: "looters", email: "", status: "approved" as DenStatus } };
  }
  const db = await sql();
  const rows = await db.query<DenRow>(
  ...
  return { role: "user" as const, user: { id: row.id, username: row.username, email: row.email, status: row.status } };
});
...
export const denForum = createServerFn({ method: "GET" }).handler(async () => {
  ...
  const db = await sql();
  if (session.kind === "admin") {
    const dms = await db.query<{ id: string; created_at: string; author: string; body: string; kind: string }>(
