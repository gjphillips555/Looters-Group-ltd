...
export function UserButton() {
  ...
  return (
    <div className="flex items-center gap-2">
      <Link to="/account" className="flex items-center gap-2" title={label}>
        {user.profileImageUrl ? (
          ...
          <span className="grid h-10 w-10 place-items-center rounded-full bg-ink text-sm font-semibold text-cream">
            {initial}
          </span>
        )}
      ...
      {authEnabled && (
        <button
          ...
          className="hidden cursor-pointer text-xs text-muted underline-offset-4 hover:underline sm:inline"
        >
          Sign out
        </button>
      )}
